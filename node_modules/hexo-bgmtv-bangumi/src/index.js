...
async function fixData(id){
  ...
  return {
      ...
      totalCount: hastotalCount && ($('span', '#infobox')[1]?.next.data !== '*') ? $('span', '#infobox')[1]?.next.data + '话' : '未知',
...
async function getBangumiCDN(idlist){
    ...
    const response = await axios.all(
        idlist.map(subjectId => (axios.get(url + `${parseInt(parseInt(subjectId) / 100)}/${subjectId}.json`)).catch(err => err)))        
    //console.log('finish getBangumiCDN')
    return response
}
